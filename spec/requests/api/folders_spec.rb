...
describe "Folders API" do
  ...
  context "when is the space's root folder" do
    subject { FactoryBot.create(:root_folder, space: space) }
  ...
  context "when is a common folder" do
    subject { FactoryBot.create(:complete_folder, space: space) }
    ...
    context "when GET /api/folders/:folder_id/folders" do
      let!(:folders) do
        (1..4).collect { FactoryBot.create(:folder, parent: subject) }
      end
    ...
    context "when GET /api/spaces/:space_id/folders" do
      let!(:folders) do
        (1..4).collect { FactoryBot.create(:folder, space: space) }
      end
  ...
  context "when PUT /api/folders/:id" do
    subject do
      FactoryBot.create(:complete_folder, space: space, parent: space.root_folder)
    end
