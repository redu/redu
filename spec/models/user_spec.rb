...
describe User do
  ...
  context "validations" do
    ...
    context "email" do
      ...
      context "uniqueness" do
        let!(:already_registered) { FactoryBot.create(:user, email: "first@mail.com") }

        it "should not be valid if already there is a user with same email" do
          u = FactoryBot.build(:user, email: "first@mail.com")
          u.should_not be_valid
          u.errors[:email].should_not be_empty
        end
      ...
      context "format" do
        it "should not be valid when e-mail does not have @" do
          u = FactoryBot.build(:user, email: "invalid.inv")
          u.should_not be_valid
          u.errors[:email].should_not be_empty
        end
      ...
      context "length" do
        it "should not be valid when email has less than 3 caracters" do
          u = FactoryBot.build(:user, email: "i@")
          u.should_not be_valid
          u.errors[:email].should_not be_empty
        end

        it "should not be valid when email has more than 100 caracters" do
          u = FactoryBot.build(:user, email: "#{SecureRandom.hex(92)}@mail.com")
          u.should_not be_valid
          u.errors[:email].should_not be_empty
        end
      end
      ...
      context "confirmation" do
        it "should not be valid when email and email_confirmation are " \
          "different" do
          u = FactoryBot.build(:user, email: "email@email.com",
    ...
    it "validates mobile phone format" do
      u = FactoryBot.build(:user, mobile: "21312312")
      u.should_not be_valid
    ...
    context "login" do
      ...
      context "uniqueness" do
        let!(:already_registered) { FactoryBot.create(:user, login: "first_here") }

        it "should not be valid if already there is a user with same login" do
          u = FactoryBot.build(:user, login: "first_here")
          u.should_not be_valid
          u.errors[:login].should_not be_empty
        end
      ...
      context "exclusion" do
        it "should not be valid when login is a reserved login" do
          ...
          u = FactoryBot.build(:user, login: login)
          u.should_not be_valid
          u.errors[:login].should_not be_empty
        end
      end
      ...
      context "length" do
        it "should not be valid when login has less than 5 letters" do
          u = FactoryBot.build(:user, login: "nick")
          u.should_not be_valid
          u.errors[:login].should_not be_empty
        end

        it "should not be valid when login has more than 20 letters" do
          u = FactoryBot.build(:user, login: "my_super_giant_login_")
          u.should_not be_valid
          u.errors[:login].should_not be_empty
        end
      end
      ...
      context "format" do
        it "should not be valid when login has dots" do
          u = FactoryBot.build(:user, login: "my.login")
          u.should_not be_valid
          u.errors[:login].should_not be_empty
        end
      ...
      context "if password_required?" do
        ...
        context "length" do
          it "should not be valid when password has less than 6 letters" do
            u = FactoryBot.build(:user, password: "passw",
          ...
          it "should not be valid when password has more than 20 letters" do
            u = FactoryBot.build(:user, password: "super_giant_password_",
    ...
    context "humanizer" do
      it "should not be valid when enabling humanizer (User#enable_humanizer)" do
        ...
        FactoryBot.build(:user).should_not be_valid
      end
    end
  end
  ...
  context "associations" do
    ...
    it "retrieves subjects that are finalized" do
      ...
      subjects_finalized = (1..3).collect { FactoryBot.create(:subject, owner: subject,
                                                    space: space,
                                                    finalized: true) }
      subjects = (1..3).collect { FactoryBot.create(:subject, owner: subject,
                                          space: space) }
  ...
  context "finders" do
    it "retrieves recent users" do
      users = (1..3).collect { |n| FactoryBot.create(:user, created_at: n.hour.ago) }
      User.recent.should == users
    end

    xit "retrieves active users" do
      active_users = (1..3).collect { |n| FactoryBot.create(:user,
                                                  activated_at: 1.day.ago) }
      users = (1..3).collect { |n| FactoryBot.create(:user) }
    ...
    it "retrieves course invitations" do
      courses = (0..3).collect { FactoryBot.create(:course) }
    ...
    it "retrieves a user by name, login or email" do
      users = []
      users << FactoryBot.create(:user, first_name: "Guilherme")
      users << FactoryBot.create(:user, login: "guilherme")
      users << FactoryBot.create(:user, email: "guiocavalcanti@redu.com.br")
    ...
    context 'when a user has multiple spaces in the end of his name' do
      before do
        # Usuários old style
        @tarci = FactoryBot.build(:user, first_name: "TARCISIO   ",
                              last_name: "COUTINHO")
    ...
    it "retrieves the 5 most popular users (more friends)" do
      @popular = (1..3).collect { |i| FactoryBot.create(:user, friends_count: 20 + i) }
      @less_popular = (1..3).collect {|i| FactoryBot.create(:user, friends_count: 10 - i) }
      @not_popular = (1..5).collect {|i| FactoryBot.create(:user, friends_count: 3) }
    ...
    it "retrieves users with email domain like 'redu.com.br'" do
      @hotmail_users = (1..3).collect do |n|
        FactoryBot.create(:user, email: "#{n}@hotmail.com")
      end

      @gmail_users =  (1..3).collect do |n|
        FactoryBot.create(:user, email: "#{n}@gmail.com")
      end

      @redu_users =  (1..3).collect do |n|
        FactoryBot.create(:user, email: "#{n}@redu.com")
      end
    ...
    it "retrieves all users except the specified users" do
      users = (1..10).collect { FactoryBot.create(:user) }
    ...
    it "retrieves all colleagues (same course but not friends or pending friends)" do
      ...
      env = FactoryBot.create(:environment, owner: owner)
      colleagues1 = (1..10).collect { FactoryBot.create(:user) }
      friends1 = (1..5).collect { FactoryBot.create(:user) }
      course1 = FactoryBot.create(:course, environment: env, owner: owner)

      colleagues2 = (1..10).collect { FactoryBot.create(:user) }
      friends2 = (1..5).collect { FactoryBot.create(:user) }
      course2 = FactoryBot.create(:course, environment: env, owner: owner)

      pending_friends = (1..10).collect { FactoryBot.create(:user) }
    ...
    it "retrieves all friends of friends (exclude pending friends)" do
      vader = FactoryBot.create(:user, login: "darth_vader")
      luke = FactoryBot.create(:user, login: "luke_skywalker")
      leia = FactoryBot.create(:user, login: "princess_leia")
      han_solo = FactoryBot.create(:user, login: "han_solo")
      yoda = FactoryBot.create(:user, login: "yodaaa")
    ...
    it "should retrieves all recipients passing a set of reccipients ids " do
      vader = FactoryBot.create(:user, login: "vaderr")
      luke = FactoryBot.create(:user, login: "luke_skywalker")
      leia = FactoryBot.create(:user, login: "princess_leia")
      han_solo = FactoryBot.create(:user, login: "han_solo")
    ...
    it "retrieves all subjects ids from your lectures" do
      @lecture = FactoryBot.create(:lecture, owner: subject)
  ...
  context "when recommending friends" do
    context "when does not have friends" do
      it "retrieves five contacts" do
        new_user = FactoryBot.create(:user, email: "user@example.com")
        teachers = (1..10).collect { FactoryBot.create(:user) }
        populars = (1..20).collect { |n| FactoryBot.create(:user, friends_count: 23 + n) }
        same_domain = FactoryBot.create(:user, email: "user2@example.com")
      ...
      it "the user is not included" do
        new_user = FactoryBot.create(:user, email: "user@example.com")
        populars = (1..2).collect { |n| FactoryBot.create(:user, friends_count: 23 + n) }
  ...
  context "when exists compound statuses" do
    before do
      ...
      ActiveRecord::Observer.with_observers(
        ...
        :friendship_observer,
        :status_observer) do
          @friends = 3.times.collect { FactoryBot.create(:user) }
  ...
  it "verifies if a profile is complete" do
    subject = FactoryBot.create(:user, gender: 'M', description: "Desc")
  ...
  it "deactivates his account" do
    subject = FactoryBot.create(:user, created_at: 40.days.ago,
                      activated_at: 1.day.ago)
    subject.deactivate
  ...
  xit "verifies if he can activate his account" do
    ...
    subject = FactoryBot.create(:user, activated_at: 1.day.ago)
    subject.can_activate?.should == false

    subject = FactoryBot.create(:user, created_at: 31.days.ago,
                      activated_at: nil)
    subject.can_activate?.should == false
  end
  ...
  context "#get_association_with" do
    it "retrieves his association with a thing" do
      ...
      subject_entity = FactoryBot.create(:subject, owner: subject,
                                          space: space, finalized: true)
      subject_entity.enroll
      subject.get_association_with(subject_entity).
        should == subject.enrollments.last

      lecture_entity = FactoryBot.create(:lecture, subject: subject_entity,
                                          owner: subject)
  ...
  it "verifies if he is environment admin of a thing" do
    environment = FactoryBot.create(:environment, owner: subject)
    environment2 = FactoryBot.create(:environment)
  ...
  it "should retrieve educations most important \
    ...
    edu1 = FactoryBot.create(:education, user: subject,
                   educationable: FactoryBot.create(:high_school))
    edu2 = FactoryBot.create(:education, user: subject,
                   educationable: FactoryBot.create(:higher_education))
    edu3 = FactoryBot.create(:education, user: subject,
  ...
  context "when application validation fail" do
    it "should prevent duplicate logins" do
      @duplicate = FactoryBot.build(:user, login: subject.login)
      expect {
