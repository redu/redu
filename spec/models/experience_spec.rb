...
describe Experience do
  subject { FactoryGirl.create(:experience) }
  ...
  context "validations" do
    it "validates presence of end_date if it is not a current experience" do
      @experience = FactoryGirl.build(:experience, :current => false,
                                  :end_date => nil)
    ...
    it "validates absense of end_date if it is a current experience" do
      @experience = FactoryGirl.build(:experience, :current => true,
  ...
  context "finders" do
    before do
      ...
      @experience1 = FactoryGirl.create(:experience, :user => @user,
                             :start_date => 6.month.ago,
                             :end_date => 3.month.ago)
      @experience2 = FactoryGirl.create(:experience, :user => @user, :current => true,
                             :start_date => 10.month.ago,
                             :end_date => nil)

      @experience3 = FactoryGirl.create(:experience, :user => @user, :current => true,
                             :start_date => 6.month.ago,
                             :end_date => nil)
    end
