...
shared_examples_for "a billable" do
  ...
  context "when dealing with current plan" do
    ...
    it "should retrieve the current plan" do
      plan1 = FactoryBot.create(:plan, :billable => subject, :current => false)
      plan2 = FactoryBot.create(:plan, :billable => subject)
      subject.reload
    ...
    it "should store the plan as current" do
      plans = (1..2).collect do
        FactoryBot.create(:plan, :billable => subject, :current => false)
      end
      subject.reload

      plan1 = FactoryBot.build(:plan)
      subject.plan = plan1
      subject.save
  ...
  it "should stores itself ond plan and self destroy" do
    FactoryBot.create(:plan, :billable => subject)
