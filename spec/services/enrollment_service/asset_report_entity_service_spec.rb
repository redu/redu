...
module EnrollmentService
  module Jobs
    describe AssetReportEntityService do
      ...
      let(:subjects) { 2.times.map { FactoryBot.create(:subject, space: nil) } }
      let!(:lectures) do
        subjects.map do |subj|
          3.times.map { FactoryBot.create(:lecture, subject: subj, owner: subj.owner) }
        end.flatten
      end

      before do
        EnrollmentEntityService.new(subject: subjects).
          create(users: FactoryBot.create_list(:user, 3))
      end
        ...
        let!(:asset_reports) do
          lectures.map do |l|
            l.subject.enrollments.map do |enrollment|
              FactoryBot.create(:asset_report, enrollment: enrollment, lecture: l)
            end.flatten
          end.flatten
        end
        let(:enrollments) { subjects.map(&:enrollments).flatten }
      ...
      describe "#get_asset_reports_for" do
        let(:asset_reports) do
          lectures.map { |l| FactoryBot.create(:asset_report, lecture: l) }
        end
