...
describe "AssetReport Ability" do
  ...
  context "when not related to the asset report" do
    let(:asset_report) { FactoryBot.create(:asset_report) }
  ...
  context "when related to the asset report" do
    let(:enrollment) { FactoryBot.create(:enrollment, :user => user) }
    let(:asset_report) { FactoryBot.create(:asset_report, :enrollment => enrollment) }
