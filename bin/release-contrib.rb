...
|-|-|-|-|-|
HEADER
CSV.parse(`git shortlog HEAD...$(git describe --abbrev=0) -sn`).each do |line|
  commits_number, author = line.first.split("\t")
  commits_number = commits_number.gsub(/\s+/, '')

  diff = `git log --shortstat --author="#{author}" | grep -E "fil(e|es) changed" | awk '{files+=$1; inserted+=$4; deleted+=$6} END {print files, "|", inserted, "|", deleted, "|" }'`.gsub(/\n/, '')
